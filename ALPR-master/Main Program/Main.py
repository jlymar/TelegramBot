...
if __name__ == "__main__":
    ...
    for f in os.listdir():
        ...
        for t in range(len(y_pred)):
            ...
            if y_pred[t] == y_test[t]:
                ...
                incorrectLiterals.append(y_test[t])
        accuracy = (score*100)/length
        avarageAccuracy += accuracy
        if accuracy == 100:
            correctPlates+=1
        new = 'Accuracy at the '+str(i)+' th image '+f+ ' is :'+str(accuracy)
        print(new,'\n','The count is: ',count)
    ...
    print(result)
    print(correctPlates)
    avarageAccuracy = avarageAccuracy/imgNum
    print(avarageAccuracy)
    print('incorrectLiterals', incorrectLiterals)
    #main('OS269DT.jpg')
    
    fdist1 = nltk.FreqDist(incorrectLiterals)
